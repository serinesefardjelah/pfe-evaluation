...
def process_all_csvs(directory="repo_stats"):
    ...
    for file_path in all_files:
        try:
            ...
            # Convert 'Failed Count' to numeric, coercing errors to NaN, then fill with 0 and convert to int
...
def analyze_mutants(full_df):
    ...
    for repo, group in full_df.groupby('Repo Name'):
        ...
        # === CHANGE: Calculate duplicates among all mutants ===
        mutant_failures = {}
        for _, row in group.iterrows():  # Use entire group, not just killed_group
        ...
        # === Subsumption still calculated only among killed mutants ===
        ...
        # === Unique is still based on killed mutants ===
        unique_count = total_killed - len(subsumed) - len(subsuming)  # no longer subtracting duplicates
...
def create_plots(results_df):
    ...
    results_df['% Duplicates'] = results_df['duplicate_mutants'] / results_df['total_mutants'] * 100
